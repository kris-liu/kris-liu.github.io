...
3. 新加入的节点可以自动部署，无须提供手动备份，维护起来很方便，数据库故障切换很容易。

##### 缺点：

1. 性能低，任何更新事务都需要全局验证通过，才会在每个节点库上执行。集群性能受限于性能最差的节点，也就是经常说的短板效应，尤其是异地多活场景下，需要跨机房数据实时同步。

2. 新加入的节点开销大，需要复制完整的数据。采用SST传输开销太大。

3. 只支持innodb存储引擎的表。

4. 没有表级别的锁定，执行DDL语句操作会把整个集群锁住，而且也kill不了（建议使用Osc操作，pt-online-scheme-change）
