...
* 第二阶段：提交阶段（执行阶段）：
	...
	* 如果有的参与者在第一阶段返回了失败，则进行回滚事务的操作过程：
...
* 缺点：
	...
	2. 单点问题：协调者为单点，一旦协调者发生故障，参与者会一直阻塞下去，即使重新选举一个协调者，也会造成参与者资源长时间被锁定。

	3. 数据不一致性问题：第二阶段中，假设协调者发出了事务commit的通知后发生了故障，但是因为网络问题该通知仅被一部分参与者收到并执行了commit操作，其余的参与者则因为没有收到通知一直处于阻塞状态，这时候就产生了数据的不一致性。
...
* 第三阶段：doCommit
	...
	* 协调者根据参与者的二阶段返回情况决定是否进行事务提交，如果所有的参与者都返回成功, 则执行事务提交的操作过程；如果超时仍没收到第三阶段的请求，那么只要参与者完成了PreCommit操作，仍会直接提交事务，因为PreCommit操作的前提是所有参与者的CanCommit都返回了可以完成事务操作，此时认为应该大概率可以提交成功：
	...
	* 如果有的参与者在第二阶段返回了失败，则进行回滚事务的操作过程：
...
* 优点：
	...
	1. 相对于2PC，解决了单点故障问题，并减少资源阻塞，因为如果参与者无法及时收到协调者信息，会默认提交commit操作，而不会长期持有锁资源，处于阻塞状态；
...
* 优点：
	...
	1. 解决了分布式应用架构场景下跨服务的事务问题，实现了面向服务的最终一致性的分布式事务，可以应用在金融场景下；

	2. 适合Try操作执行时间确定且较短的业务。

	3. 通过业务逻辑锁定解决了2PC情况下的2PL带来的长时间资源阻塞锁定问题，提高了业务并发能力；

	4. TCC的机制只要Try操作成功，Confirm必须能成功，从而可以允许二阶段进行一定的异步化处理，起到消峰的作用，可以提高业务在峰值时期的业务处理能力。
...
* 缺点：
	...
	3. 主业务服务在本地事物中需要进行RPC调用从业务服务的Try接口，远程调用时间开销会导致长时间占用DB连接，严重时DB连接池占满将导致应用无法访问DB。所以需要应用合理配置DB连接数，并控制Try接口时间和超时时间设置；

	4. 分布式事务的悬挂问题：当网络问题导致二阶段Cancel先于一阶段Try时，会导致一阶段Try预留的资源一直被锁定无法自动回滚，所以需要业务来处理分布式事务的悬挂问题。一般通过拒绝一定时间内的空回滚，等待重试来回滚；或者通过从业务服务在Try、Confirm和Cancel三个接口第一次调用时插入一条事务活动记录，用来获知三个接口的调用情况，从而防止事务悬挂发生。
